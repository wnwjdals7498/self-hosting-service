...
# fork-custom: AdminBackupView accepts both session and API-key auth
...
# fork-custom: on-demand Plane backup trigger (plane-mcp-v2-deploy, 2026-04-24)
class AdminBackupView(BaseAPIView):
    """Kick off the ``PlaneBackup`` scheduled task on demand.

    Protected by :class:`InstanceAdminPermission`. Accepts both admin-web
    session cookies and an ``X-Api-Key`` header so plane-mcp-server's
    ``trigger_backup_now`` tool can call it.
    """
    ...
    def post(self, request):
        try:
            ...
            return Response(
                {"detail": "Start-ScheduledTask timed out after 10s"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
        except FileNotFoundError:
            return Response(
                {"detail": "powershell.exe not found on server PATH"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )

        if result.returncode != 0:
            stderr_head = (result.stderr or "").splitlines()
            first_line = stderr_head[0] if stderr_head else ""
            return Response(
                {"detail": f"Failed to start PlaneBackup task: {first_line}"},
                status=status.HTTP_500_INTERNAL_SERVER_ERROR,
            )
