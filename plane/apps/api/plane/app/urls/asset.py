...
# Phase 4.1 (deploy): LocalFSStorage upload/download endpoints imported
# directly from the submodule so they stay out of plane.app.views.__init__
# (they have no consumers outside of this urls file, and the HMAC logic
# only runs when USE_LOCAL_STORAGE=1).
...
urlpatterns = [
    ...
    # Phase 4.1 (deploy): self-hosted presigned endpoints. Routes are
    # always registered; LocalFSStorage.generate_presigned_{post,url}
    # only emits URLs that resolve here when USE_LOCAL_STORAGE=1.
    path(
        "assets/local-upload/",
        LocalUploadView.as_view(),
        name="local-upload",
    ),
    re_path(
        r"^assets/local-download/(?P<path>.+)$",
        LocalDownloadView.as_view(),
        name="local-download",
    ),
]
