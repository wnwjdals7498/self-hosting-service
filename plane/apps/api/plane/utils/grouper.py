...
def issue_queryset_grouper(
    ...
    """Annotate an Issue queryset with assignee_ids / label_ids / module_ids.

    Historically these annotations were built with ``ArrayAgg`` which is a
    Postgres-only aggregate. The rewrite uses SQLite's ``json_group_array``
    exposed via the ``JsonGroupArray`` Func above; Django materialises the
    JSONField into a Python list, preserving the downstream contract used by
    ``issue_on_results`` and by serializers that read these attributes from
    ``.values(...)`` output.
    """
    ...
    annotations_map: Dict[str, Any] = {
        "assignee_ids": Coalesce(issue_assignee_subquery, empty_json_array()),
        "label_ids": Coalesce(issue_label_subquery, empty_json_array()),
        "module_ids": Coalesce(issue_module_subquery, empty_json_array()),
    }
