...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        # Phase 4.3: LocalFSStorage has no bucket policy to update — bail
        # out before touching boto3 so USE_LOCAL_STORAGE=1 deployments can
        # still run the upstream bootstrap script verbatim.
        if getattr(settings, "USE_LOCAL_STORAGE", False):
            self.stdout.write(
                self.style.NOTICE(
                    "USE_LOCAL_STORAGE=1: skipping bucket policy update (local FS has no bucket concept)."
                )
            )
            return
