...
# Database — SQLite single-file store.
#
# Selection: SQLITE_PATH env var if set, otherwise BASE_DIR/plane.sqlite3.
# PRAGMAs (WAL, synchronous=NORMAL, busy_timeout=5000, foreign_keys=ON)
# are applied via plane.db.apps.DbConfig.ready() on every new connection,
# and BEGIN IMMEDIATE is installed for safe multi-writer behaviour (see
# phase-g / phase-f docs). Django 4.2's sqlite3 backend does not accept
# an init_command OPTIONS key, so the PRAGMAs live in the signal.
DATABASES = {
    "default": {
        "ENGINE": "django.db.backends.sqlite3",
        "NAME": os.environ.get("SQLITE_PATH", os.path.join(BASE_DIR, "plane.sqlite3")),
        "OPTIONS": {"timeout": 30},
    }
}
...
# Storage Settings
# Phase 4.1: USE_LOCAL_STORAGE=1 swaps the default backend to the
# LocalFSStorage adapter (self-hosted, HMAC-signed). Default 0 preserves
# the upstream S3/MinIO path for any deployment that keeps object storage.
...
if USE_LOCAL_STORAGE:
    ...
    # HMAC key for presigned upload/download signatures. Kept separate
    # from SECRET_KEY so that rotating session/CSRF keys does not
    # invalidate every outstanding signed URL.
...
# ---------------------------------------------------------------------------
# SQLite fork-local patch (handoff §7b): upstream Plane has 30+ sites using
#   .annotate(count=Func(F("id"), function="Count"))
# without an explicit output_field. Django infers UUIDField from F("id"), so
# SQLite's convert_uuidfield_value runs on the aggregate integer result and
# crashes with "'int' object has no attribute 'replace'" (uuid.UUID(<int>)
# attempts hex.replace internally). Fix on the backend so int values pass
# through unchanged; genuine UUIDs still go through the original converter.
try:
    from django.db.backends.sqlite3.operations import DatabaseOperations as _SQLiteOps

    _plane_orig_convert_uuid = _SQLiteOps.convert_uuidfield_value

    def _plane_safe_convert_uuid(self, value, expression, connection):
        if isinstance(value, int):
            return value
        return _plane_orig_convert_uuid(self, value, expression, connection)

    _SQLiteOps.convert_uuidfield_value = _plane_safe_convert_uuid
except ImportError:
    pass
